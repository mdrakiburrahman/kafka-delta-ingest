...
cargo run --features azure ingest dmv abfss://...@...dfs.core.windows.net/synapse/workspaces/.../temp/demo-table-rust \
  ...
  --kafka_setting "sasl.password=Endpoint=sb://....servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=..." \
  ...
  --kafka_setting "fetch.min.bytes=200000" \
  --kafka_setting "fetch.max.bytes=209715200" \
...
```
docker build -t kdi:0.1 . -f Dockerfile
```
